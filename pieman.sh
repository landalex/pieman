...
"${IMAGE_CLASSIC}")
    ...
    if [[ "${DEVICE}" == "opi-pc-plus" ]]; then
        dd if="${TOOLSET_FULL_PATH}/uboot-${UBOOT_VER}/u-boot-sunxi-with-spl.bin" of="${LOOP_DEV}" bs=1024 seek=8
        sync
    fi
    ...
    if [[ "${DEVICE}" == "opi-pc-plus" ]]; then
        "${TOOLSET_FULL_PATH}/uboot-${UBOOT_VER}"/mkimage -C none -A arm -T script -d "${PIEMAN_DIR}"/files/opi/boot.cmd "${BOOT}"/boot.scr
    fi
